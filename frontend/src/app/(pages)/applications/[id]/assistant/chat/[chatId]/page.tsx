  ...
  return (
    <div className="flex flex-1 items-center justify-center">
      <div className="relative flex h-[30px] items-center justify-center">
        ...
        <h1
          ref={textRef}
          className="absolute font-serif text-2xl font-light whitespace-nowrap text-gray-900"
          style={{
            left: "50%",
  ...
  return (
    <div className="relative z-10 w-0 shrink-0">
      <div
        ...
        className="absolute inset-y-0 -right-2 -left-2 flex cursor-col-resize items-stretch justify-center"
      >
        {isDragging && <div className="w-1 bg-blue-500 transition-colors" />}
      </div>
    </div>
  );
}

export default function ApplicationAssistantChatPage({ params }: Props) {
  const { id: applicationId, chatId } = use(params);
  ...
  const [initialMessages] = useState<LukeMessage[]>(newChatMessages ?? []);
  ...
  // Whenever the assistant mutates application documents — creating a new
  // doc, creating a new version via edit_document, or replicating a doc —
  // refresh the application so the explorer picks up the new/changed files
  ...
  useEffect(() => {
    if (!applicationMutationSignature) return;
    getApplication(applicationId)
      .then(setApplication)
      .catch(() => {});
  ...
  useEffect(() => {
    ...
    getChat(chatId)
      ...
      .catch(() => router.replace(`/applications/${applicationId}?tab=assistant`))
      .finally(() => setChatLoaded(true));
  }, [chatId, applicationId, router, setMessages]);
    ...
    setTabs((prev) => {
      ...
      if (existing) {
        if (versionId !== undefined && existing.versionId !== versionId) {
          return prev.map((t) => (t.documentId === docId ? { ...t, versionId } : t));
        }
        return prev;
      }
      return [...prev, { documentId: docId, filename, quotes, versionId }];
    });
  ...
  const handleDocClick = (doc: LukeDocument) => {
    openTab(doc.id, doc.filename);
  };

  const handleCitationClick = (citation: LukeCitationAnnotation) => {
    openTab(citation.document_id, citation.filename, expandCitationToEntries(citation));
  };
  ...
  const handleEditViewClick = (ann: LukeEditAnnotation, filename: string) => {
  ...
  async function handleDeleteChat() {
    ...
    try {
      ...
      router.push(`/applications/${applicationId}?tab=assistant`);
    } finally {
  ...
  async function uploadFiles(files: File[]) {
    ...
    try {
      const uploaded = await Promise.all(
        files.map((f) => uploadApplicationDocument(applicationId, f)),
      );
      setApplication((prev) => {
        if (!prev) return prev;
        return {
  ...
  const handleExplorerFileDrop = async (e: React.DragEvent) => {
    ...
    // Internal doc/folder moves are handled inside ApplicationExplorer (stopPropagation)
  };
  ...
  const handleCreateFolder = async (parentId: string | null, name: string) => {
    ...
    setApplication((prev) =>
      prev ? { ...prev, folders: [...(prev.folders ?? []), folder] } : prev,
    );
  };
      ...
      prev
        ? {
            ...
            folders: (prev.folders ?? []).map((f) =>
              f.id === folderId ? { ...f, parent_folder_id: targetFolderId } : f,
            ),
          }
        : prev,
    );
    await moveSubfolderToFolder(applicationId, folderId, targetFolderId);
  };
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex shrink-0 items-center justify-between px-8 py-4">
        <div className="flex items-center gap-1.5 font-serif text-2xl font-medium">
          ...
          <span className="text-gray-300">›</span>
          <button
            onClick={() => router.push(`/applications/${applicationId}?tab=assistant`)}
            className="text-gray-500 transition-colors hover:text-gray-700"
          >
            Assistant
          </button>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            title="Delete chat"
            className="flex items-center justify-center p-1.5 text-gray-500 transition-colors hover:text-red-600 disabled:opacity-40"
          >
            {deletingChat ? (
              <Loader2 className="h-4 w-4 animate-spin" />
            ) : (
      ...
      <div className="flex min-h-0 flex-1 overflow-hidden border-t border-gray-200">
        ...
        <div className="flex min-w-0 flex-1 flex-col border-r border-gray-200">
          ...
          <div
            ref={tabBarRef}
            className="flex h-10 min-w-0 shrink-0 [scrollbar-width:none] items-end overflow-x-auto border-b border-gray-200 [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden"
          >
            {tabs.length === 0 ? (
              <span className="self-center px-4 text-xs text-gray-700">Document Viewer</span>
            ) : (
              tabs.map((tab) => {
                ...
                // of the application state so the tab shows V#
                // whenever the doc has been edited.
                const versionNumber = (application?.documents ?? []).find(
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={`truncate text-xs ${isActive ? "font-medium text-gray-900" : "text-gray-500"}`}
                    >
                      {tab.filename}
                    </span>
                    {showVersionBadge && (
            ...
            {activeTab ? (
              isDocxTab(activeTab.filename) ? (
                <DocxView
                  ...
                  highlightEdit={
                    editScrollTarget && editScrollTarget.documentId === activeTab.documentId
                      ? editScrollTarget
                      : null
                  }
                  onReady={() => handleDocxReady(activeTab.documentId)}
                  ...
                  onScrollChange={(top) => handleTabScrollChange(activeTab.documentId, top)}
                  rounded={false}
                  bordered={false}
                />
              ) : (
              ...
              <div className="flex h-full items-center justify-center bg-gray-100 px-8">
                <div className="space-y-3 text-center">
                  ...
                  <p className="font-serif text-base text-gray-500">
                    Pro tip: Drag a document from the Application Explorer to the Assistant to
