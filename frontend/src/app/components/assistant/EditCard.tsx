...
function findMatch(
  ...
  if (opts.w_id) {
    ...
    const byId = container.querySelector(`${tag}[data-w-id="${opts.w_id}"]`) as HTMLElement | null;
    if (byId) return byId;
  }
  const text = opts.text ?? "";
  const target = normalizeText(text);
  if (!target) return null;
  const candidates = Array.from(container.querySelectorAll(tag)) as HTMLElement[];
  const byText =
    candidates.find((el) => normalizeText(el.textContent ?? "") === target) ??
...
export function applyOptimisticResolution(
  ...
  const keep = (el: HTMLElement) => {
    ...
    const snapshot = {
      color: [el.style.getPropertyValue("color"), el.style.getPropertyPriority("color")] as const,
      bg: [
...
interface Props {
   ...
   */
  isReloading?: boolean;
  onViewClick?: (ann: LukeEditAnnotation) => void;
  /**
...
export function EditCard({
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
      {annotation.reason && <p className="mb-2 text-xs text-gray-500">{annotation.reason}</p>}
      <div className="rounded-md border border-gray-200 bg-white px-2 py-2 font-serif text-sm leading-relaxed">
        {annotation.inserted_text && (
          <span className="text-green-700">{annotation.inserted_text}</span>
        )}
        {annotation.deleted_text && (
          <span className="text-red-600 line-through">{annotation.deleted_text}</span>
        )}
      </div>
      <div className="mt-3 flex gap-2">
        <button
          ...
          className="rounded border border-gray-900 bg-gray-900 px-2 py-1 text-xs text-white hover:bg-gray-800 disabled:opacity-50"
        >
        ...
        <button
          ...
          disabled={inFlight || resolved}
          className="rounded border border-gray-200 bg-white px-2 py-1 text-xs text-gray-700 hover:bg-gray-100 disabled:opacity-50"
        >
        ...
        {onViewClick && (
          <button
            ...
            className="ml-auto rounded border border-gray-200 bg-white px-2 py-1 text-xs text-gray-700 hover:bg-gray-100 disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:bg-white"
          >
            View
          </button>
        )}
