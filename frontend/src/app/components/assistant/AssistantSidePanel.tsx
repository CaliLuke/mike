...
import type { LukeCitationAnnotation, LukeEditAnnotation } from "../shared/types";
...
export function AssistantSidePanel({
  ...
  const [panelWidth, setPanelWidth] = useState(() =>
    typeof window !== "undefined" ? Math.round((window.innerWidth - MAX_WIDTH_OFFSET) / 2) : 600,
  );
  ...
  return (
    <div
      ref={panelRef}
      className="relative flex h-full shrink-0 flex-col border-l border-gray-200 bg-white shadow-[-4px_0_12px_rgba(0,0,0,0.02)]"
      style={{ width: panelWidth }}
    >
      {/* Drag handle */}
      <div
        onMouseDown={onMouseDown}
        className="absolute top-0 left-0 z-10 h-full w-1 cursor-col-resize transition-colors hover:bg-blue-400"
        style={{ marginLeft: -2 }}
      />

      {/* Tab strip (Chrome-style) */}
      <div className="flex items-end gap-1 bg-gray-100 pt-2 pr-2">
        <div className="flex flex-1 items-end gap-1 overflow-x-auto pr-2 pl-2">
          {tabs.map((tab) => {
            ...
            return (
              <div
                ...
                className={`group relative flex h-8 max-w-[220px] min-w-0 cursor-pointer items-center gap-1.5 rounded-t-lg pr-1.5 pl-3 transition-colors select-none ${
                  isActive
                    ? "bg-white text-gray-800 before:absolute before:bottom-0 before:-left-2 before:h-2 before:w-2 before:bg-[radial-gradient(circle_at_top_left,transparent_8px,white_9px)] before:content-[''] after:absolute after:-right-2 after:bottom-0 after:h-2 after:w-2 after:bg-[radial-gradient(circle_at_top_right,transparent_8px,white_9px)] after:content-['']"
        ...
        <button
          onClick={onCloseAll}
          className="mb-1 ml-1 shrink-0 rounded-lg p-1.5 text-gray-400 hover:bg-gray-200 hover:text-gray-700"
          title="Close panel"
        >
          <X className="h-4 w-4" />
        </button>
      ...
      <div className="relative min-h-0 flex-1">
        {tabs.map((tab) => {
            ...
            tab.kind === "citation"
              ...
              : tab.kind === "edit"
                ? {
                    kind: "edit",
                    edit: tab.edit,
                    isEditReloading: isEditReloading?.(tab.edit.edit_id) ?? false,
          ...
          return (
            <div
              key={tab.id}
              className={`absolute inset-0 flex flex-col ${isActive ? "" : "pointer-events-none invisible"}`}
            ...
            >
              <DocPanel
                ...
                mode={mode}
                isReloading={isEditorReloading?.(tab.documentId) ?? false}
                warning={tab.warning ?? null}
                onWarningDismiss={() => onWarningDismiss?.(tab.id)}
