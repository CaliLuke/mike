...
import type { CitationQuote, LukeCitationAnnotation, LukeEditAnnotation } from "./types";
import { displayCitationQuote, expandCitationToEntries, formatCitationPage } from "./types";
...
export type DocPanelMode =
  | { kind: "document" }
  | { kind: "citation"; citation: LukeCitationAnnotation }
  | {
      kind: "edit";
      edit: LukeEditAnnotation;
      /**
    ...
    <div className="pt-2 pb-3">
      ...
      <div className="w-full rounded-md border border-gray-200 bg-gray-50 px-2 py-2">
        <p className="font-serif text-sm text-gray-600">
          ...
          {pagesLabel && <span className="ml-1 text-gray-400">({pagesLabel})</span>}
        </p>
      </div>
    </div>
  );
...
function TrackedChangeHeader({
  ...
  return (
    <div className="pt-2 pb-3">
      ...
      {edit.reason && <p className="mb-2 text-xs text-gray-500">{edit.reason}</p>}
      <div className="w-full rounded-md border border-gray-200 bg-gray-50 px-2 py-2">
        <div className="font-serif text-sm leading-relaxed">
          {edit.inserted_text && <span className="text-green-700">{edit.inserted_text}</span>}
          {edit.deleted_text && (
  ...
  const handle = useCallback(
    async (verb: "accept" | "reject") => {
      ...
      try {
        ...
        console.error("[DocPanel] optimistic update threw", e);
      }
      try {
        const resp = await fetch(
          `${API_BASE}/single-documents/${edit.document_id}/edits/${edit.edit_id}/${verb}`,
          {
            method: "POST",
          },
        );
        ...
        try {
          ...
          console.error("[DocPanel] revert threw", revertErr);
        }
        onError?.({
          editId: edit.edit_id,
          documentId: edit.document_id,
  ...
  return (
    <div className="flex items-center gap-2">
      <button
        ...
        className="inline-flex items-center gap-1 rounded-lg border border-gray-900 bg-gray-900 px-2 py-1.5 text-xs font-medium text-white hover:bg-gray-800 disabled:cursor-not-allowed disabled:opacity-50"
      >
      ...
      <button
        ...
        disabled={inFlight || resolved}
        className="inline-flex items-center gap-1 rounded-lg border border-gray-200 bg-white px-2 py-1.5 text-xs font-medium text-gray-700 hover:bg-gray-100 disabled:cursor-not-allowed disabled:opacity-50"
      >
...
function DownloadButton({
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      const resp = await fetch(`${API_BASE}/single-documents/${documentId}/docx${qs}`);
      if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
  ...
  return (
    <button
      onClick={handleClick}
      disabled={spinning}
      className="inline-flex items-center gap-1 rounded-lg border border-gray-200 px-2 py-1.5 text-xs font-medium text-gray-600 hover:bg-gray-100 hover:text-gray-800 disabled:cursor-not-allowed disabled:opacity-50"
    >
      {spinning ? (
