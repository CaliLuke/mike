...
function findEditElement(
  ...
  return (
    ...
    candidates.find((el) => normalize(el.textContent ?? "").includes(target)) ??
    null
  );
}
...
async function tagWIdsOnRenderedDom(
  ...
  try {
    ...
    const resp = await fetch(`${API_BASE}/single-documents/${documentId}/tracked-change-ids${qs}`);
    if (!resp.ok) {
      console.warn("[DocxView] tracked-change-ids fetch failed", resp.status);
      return;
    }
...
export function DocxView({
  ...
  const quoteKey = useMemo(() => (quotes ?? []).map((q) => q.quote).join("||"), [quotes]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        await tagWIdsOnRenderedDom(containerEl, documentId, versionId ?? null);
        if (cancelled) return;
        // Scale to fit before scrolling so offsets are computed
        // against the post-zoom layout.
        applyDocxScale();
        requestAnimationFrame(() => {
          if (!scrollRef.current || thisRender !== renderKeyRef.current) return;
          ...
          if (pendingHighlight) {
            scrollToHighlight(containerEl, scrollRef.current, pendingHighlight);
            // Highlight quotes too, but don't override the edit scroll
            if (pendingQuotes?.length) {
              for (const q of pendingQuotes) highlightDocxQuote(containerEl, q.quote);
            }
          } else if (
            pendingQuotes &&
            applyQuoteHighlights(containerEl, scrollRef.current, pendingQuotes)
          ) {
  ...
  return (
    ...
    >
      ...
      <div
        ref={scrollRef}
        className="docx-view-scroll flex-1 overflow-auto bg-gray-100 px-5 pt-5 pb-3"
