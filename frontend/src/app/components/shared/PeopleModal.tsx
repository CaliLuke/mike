...
export function PeopleModal({
  ...
  const isOwnerEmail = !!ownerEmail && trimmedNewEmail === ownerEmail.toLowerCase();
  const canAdd = isValidEmail && !alreadyShared && !isOwnerEmail && busy === null;
  ...
  async function handleAdd() {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : "Couldn't add the member. Try again.");
    } finally {
      setBusy(null);
    }
  }
  ...
  async function handleRemove(email: string) {
    ...
    setError(null);
    try {
      const next = sharedWith.filter((e) => e.toLowerCase() !== email.toLowerCase());
      await onSharedWithChange(next);
    } catch (e) {
      setError(e instanceof Error ? e.message : "Couldn't remove the member. Try again.");
    } finally {
      setBusy(null);
      setRemovingEmail(null);
    }
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/10 backdrop-blur-xs">
      <div className="flex h-[600px] w-full max-w-2xl flex-col rounded-2xl bg-white shadow-2xl">
          ...
          <div className="px-4 pt-1 pb-2">
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                disabled={!canAdd}
                title="Add member"
                className="inline-flex items-center justify-center rounded-lg border border-gray-900 bg-gray-900 p-2 text-white hover:bg-gray-800 disabled:cursor-not-allowed disabled:opacity-50"
              >
                {busy === "add" ? (
            ...
            {alreadyShared && trimmedNewEmail && (
              <p className="mt-1.5 text-xs text-gray-400">{trimmedNewEmail} already has access.</p>
            )}
            {isOwnerEmail && trimmedNewEmail && (
              <p className="mt-1.5 text-xs text-gray-400">{trimmedNewEmail} is the owner.</p>
            )}
            {trimmedNewEmail && !isValidEmail && !alreadyShared && !isOwnerEmail && (
              <p className="mt-1.5 text-xs text-gray-400">Enter a valid email.</p>
            )}
        ...
        <div className="flex items-center gap-2 px-4 pt-3 pb-1">
          <h3 className="text-xs font-medium text-gray-500">People with Access</h3>
          {peopleLoading && <Loader2 className="h-3 w-3 animate-spin text-gray-400" />}
        </div>
          ...
          {roster.length === 0 ? (
            ...
            <ul className="divide-y divide-gray-100 [&>li:nth-child(2)]:border-t-0">
              {roster.map((entry) => {
                ...
                const isRemoving = busy === "remove" && removingEmail === entry.email;
                const primary = entry.display_name?.trim() || entry.email;
                const showSecondary = !!entry.display_name?.trim() && primary !== entry.email;
                return (
                  <li key={`${entry.role}-${entry.email}`} className="flex items-center gap-3 py-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-sm text-gray-800">
                        {primary}
                        {isYou && <span className="ml-1.5 text-xs text-gray-400">(You)</span>}
                        {entry.role === "owner" && (
                          <span className="ml-1.5 text-[10px] text-gray-400">Owner</span>
                        )}
                      </p>
                      {showSecondary && (
                    ...
                    {entry.role === "member" && onSharedWithChange && (
                      <button
                        ...
                        disabled={busy !== null}
                        title="Remove access"
                        className="inline-flex items-center gap-1 self-center rounded px-2 py-1 text-xs text-gray-500 hover:bg-red-50 hover:text-red-600 disabled:opacity-50"
                      >
