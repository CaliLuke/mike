...
export function DocView({
  ...
  const quoteList: QuoteEntry[] = useMemo(() => {
    if (quotes?.length) return quotes.map((q) => ({ page: q.page, quote: q.quote }));
  ...
  const quoteKey = quoteList.map((q) => `${q.page ?? ""}:${q.quote}`).join("|");
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      pages.forEach((p, i) => {
        const pageCenter = p.wrapper.offsetTop + p.wrapper.clientHeight / 2;
  ...
  const applyHighlights = useCallback(async (list: QuoteEntry[]): Promise<number | null> => {
    // Clear any prior highlights across all pages
    for (const p of renderedPagesRef.current) clearHighlights(p.textDivs);
    ...
    for (const entry of list) {
      ...
      if (entry.page) {
        ...
        if (target) {
          const found = await highlightQuote(target.textDivs, entry.quote);
      ...
      if (hitPage === null) {
        ...
        for (let i = 0; i < renderedPagesRef.current.length; i++) {
          ...
          const found = await highlightQuote(p.textDivs, entry.quote);
          if (found) {
            hitPage = i + 1;
            break;
          }
  ...
  function scrollToHighlightOnPage(pageNum: number) {
    ...
    const highlightEl = pageEntry.wrapper.querySelector<HTMLElement>(".pdf-text-highlight");
    if (highlightEl) {
      ...
      const containerRect = scrollEl.getBoundingClientRect();
      const targetTop = scrollEl.scrollTop + (wrapperRect.top - containerRect.top);
      scrollEl.scrollTo({
        top: Math.max(0, targetTop),
        behavior: "instant" as ScrollBehavior,
      });
  ...
  const renderPDF = useCallback(
    async (doc: PDFDocumentProxy, list: QuoteEntry[], scrollToPage?: number) => {
      ...
      const reveal = () => {
        if (scrollContainerRef.current) scrollContainerRef.current.style.opacity = "1";
      };

      const panelW = containerRef.current.clientWidth;
      const firstPage = await doc.getPage(1);
      const naturalWidth = firstPage.getViewport({ scale: 1 }).width;
      const baseScale = Math.max(0.5, (panelW - SIDE_PADDING) / naturalWidth);
  ...
  const rehighlightQuotes = useCallback(
    async (list: QuoteEntry[]) => {
      ...
      const scrollPage = targetPage ?? list.find((e) => e.page)?.page ?? null;
  ...
  useEffect(() => {
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      const next = Math.min(
        ZOOM_MAX,
        Math.max(ZOOM_MIN, Math.round(zoomRef.current * Math.exp(-delta) * 100) / 100),
      );
      ...
      debounceTimer = setTimeout(() => {
        if (pdfDocRef.current) {
          renderPDF(pdfDocRef.current, quoteListRef.current, currentPageRef.current);
        }
      }, 150);
    };
  ...
  useEffect(() => {
    ...
    const handleTouchMove = (e: TouchEvent) => {
      ...
      const next = Math.min(
        ...
        Math.max(
          ZOOM_MIN,
          Math.round(initialZoom * (getTouchDist(e.touches) / initialDist) * 100) / 100,
        ),
      );
      zoomRef.current = next;
      setZoom(next);
  ...
  if (fallbackToDocx && doc?.document_id) {
    return <DocxView documentId={doc.document_id} quotes={quotes} />;
  }
  ...
  return (
    ...
    >
      <div ref={scrollContainerRef} className="flex-1 overflow-auto bg-gray-100 px-3 pt-5 pb-3">
        {loading && (
          <div className="flex h-full items-center justify-center">
            <LukeIcon spin luke size={28} />
          </div>
        )}
        {error && (
          <div className="flex h-full items-center justify-center">
      ...
      {numPages > 0 && (
        <>
          ...
          <div className="pointer-events-none absolute bottom-4 left-4">
            <span className="flex items-center rounded-full border border-white/30 bg-white/25 px-3 py-1.5 text-xs font-medium text-gray-700 tabular-nums shadow-md backdrop-blur-md">
              {currentPage}/{numPages}
            </span>
          </div>

          {/* Zoom controls — bottom right */}
          <div className="absolute right-4 bottom-4 flex items-center gap-px rounded-full border border-white/30 bg-white/25 px-1 py-1 shadow-md backdrop-blur-md">
            <button
            ...
            <span className="w-9 text-center text-xs font-medium text-gray-600 tabular-nums select-none">
              {Math.round(zoom * 100)}%
            </span>
            <button
