...
import { listDocumentVersions } from "@/app/lib/lukeApi";
...
export function UploadNewVersionModal({ open, onClose, doc, onSubmit }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const { current_version_id, versions } = await listDocumentVersions(doc.id);
        const current = versions.find((v) => v.id === current_version_id);
        const initial = (current?.display_name && current.display_name.trim()) || doc.filename;
        if (!cancelled) {
          setName(initial);
  ...
  const accept =
    doc.file_type === "pdf"
      ? ".pdf"
      : doc.file_type === "md" || doc.file_type === "markdown" || doc.file_type === "txt"
        ? `.${doc.file_type}`
        : ".docx,.doc";
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/10 backdrop-blur-xs">
      <div className="flex w-full max-w-md flex-col rounded-2xl bg-white shadow-2xl">
        ...
        <div className="flex items-center justify-between px-5 py-4">
          <div className="text-xs text-gray-400">Upload new version · {doc.filename}</div>
          <button
            onClick={onClose}
        ...
        <div className="px-5 pb-4">
          <label className="mb-1 block text-xs font-medium text-gray-500">New version name</label>
          <input
            type="text"
            value={name}
          ...
          {stagedFile && (
            <div className="mt-2 truncate text-xs text-gray-500">
              New Version File: <span className="text-gray-700">{stagedFile.name}</span>
            </div>
          )}
        </div>
