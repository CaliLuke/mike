const PROMPT_PRESETS: Array<{
  ...
  {
    matches: /\b(compan(y|ies)|employer|employment|work history)\b/i,
    prompt: () =>
      "Extract every company or employer the document associates with the subject and the dates of that association. Write one markdown bullet per company-role pair in this shape:\n" +
      "• <Company name> — <Role> (<Start Mon YYYY> – <End Mon YYYY or 'Present'>)\n" +
      'If a date is missing or ambiguous, write "Unknown" for that field. Sort most recent first. Do not invent companies, roles, or dates that are not in the document.',
  },
  {
