...
export type PillSegment = { type: "text"; content: string } | { type: "pill"; content: string };
...
export function getPillClass(content: string, column?: ColumnConfig): string {
  ...
  if (column?.format === "tag" && column.tags?.length) {
    const idx = column.tags.findIndex((t) => t.toLowerCase() === content.toLowerCase());
    if (idx >= 0) return TAG_COLORS[idx % TAG_COLORS.length] ?? "bg-gray-100 text-gray-700";
  }
  return "bg-gray-100 text-gray-700";
}
