...
async function saveProfilePatch(patch: Record<string, string>): Promise<UserProfile> {
...
export function UserProfileProvider({ children }: { children: ReactNode }) {
  ...
  const updateOrganisation = useCallback(async (organisation: string): Promise<boolean> => {
    try {
    ...
    setProfile((prev) =>
      prev
        ? {
            ...
            creditsRemaining: MONTHLY_CREDIT_LIMIT - (prev.messageCreditsUsed + 1),
          }
        : prev,
    );
    return true;
