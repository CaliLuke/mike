...
function Blades({ ids }: { ids: Record<string, string> }) {
  return (
    <g transform="translate(250, 250)">
      {DEGREES.map((deg) => (
        <g key={deg} transform={`rotate(${deg})`} filter={`url(#${ids.shadow})`}>
          <use href={`#${ids.blade}`} fill={`url(#${ids.glassFill})`} />
          <use href={`#${ids.blade}`} fill={`url(#${ids.innerLight})`} />
          <use
            href={`#${ids.blade}`}
...
export function LukeIcon({
  ...
  const palette = error ? ERROR_PALETTE : done ? DONE_PALETTE : DEFAULT_PALETTE;
  const m = {
    shadow: `${id}-m-shadow`,
    ...
    >
      ...
      >
        <defs>
          ...
          <linearGradient id={m.glassFill} x1="0%" y1="0%" x2="100%" y2="100%">
            <stop
              offset="0%"
              style={{
          ...
          <linearGradient id={m.specular} x1="0%" y1="0%" x2="0%" y2="100%">
            <stop
              offset="0%"
              style={{
                stopColor: "#ffffff",
          ...
          <linearGradient id={m.glassBorder} x1="0%" y1="0%" x2="0%" y2="100%">
            <stop
              offset="0%"
              style={{
          ...
          <linearGradient id={m.innerLight} x1="100%" y1="0%" x2="0%" y2="100%">
            <stop
              offset="0%"
              style={{
