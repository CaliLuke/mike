...
// Server lists the application_chat service endpoint HTTP handlers.
type Server struct {
	Mounts []*MountPoint
	Stream http.Handler
...
type MountPoint struct {
	...
	Pattern string
} // New instantiates HTTP handlers for all the application_chat service
...
func (s *Server) MethodNames() []string {
	return applicationchat.MethodNames[:]
} // Mount configures the mux to serve the application_chat endpoints.
...
func (s *Server) Mount(mux loomhttp.Muxer) {
	Mount(mux, s)
} // MountStreamHandler configures the mux to serve the "application_chat"
...
func MountStreamHandler(mux loomhttp.Muxer, h http.Handler) {
	...
	mux.Handle("POST", "/applications/{applicationId}/chat", f)
} // NewStreamHandler creates a HTTP handler which loads the HTTP request and
// calls the "application_chat" service "stream" endpoint.
func NewStreamHandler(
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		v := &applicationchat.StreamEndpointInput{
			Stream:  stream,
			Payload: payload,
		}
